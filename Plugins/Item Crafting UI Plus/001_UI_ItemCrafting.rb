...
class ItemCraft_Scene
  ...
  def pbRedrawItem(index,volume)
	...
	if item.is_machine? && item.move
	  move_name = GameData::Move.get(item.move).name
	  item_display_name = "#{item.name} - #{move_name}"
	end
	textpos.push([sprintf("%s x%d", item_display_name, recipe_data.yield), @w + 98, @h + 12, 0, BASEDARK, SHADOWDARK])
    ingredients.length.times do |i|
      ingredient = ingredients[i][0]
      quantity = ingredients[i][1]
      if ingredient.is_a?(Symbol)
