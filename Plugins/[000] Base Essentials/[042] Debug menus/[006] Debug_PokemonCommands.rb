    ...
    loop do
      ...
      when 1   # Set IVs
        ...
        loop do
          ...
          ivcommands.push(_INTL("Set all to 31"))
          ivcommands.push(_INTL("Set all to 0"))
          ...
          if cmd2 < iv_id.length
            ...
            case cmd2
            when iv_id.length       # Randomise all
              GameData::Stat.each_main do |s|
                pkmn.iv[s.id] = rand(Pokemon::IV_STAT_LIMIT + 1)
              end
            when iv_id.length + 1   # Set all to 31
              GameData::Stat.each_main do |s|
                pkmn.iv[s.id] = Pokemon::IV_STAT_LIMIT
              end
            when iv_id.length + 2   # Set all to 0
              GameData::Stat.each_main do |s|
                pkmn.iv[s.id] = 0
              end
            end
    ...
    loop do
      ...
      when 0   # Make egg
        if !pkmn.egg? && (pbHasEgg?(pkmn.species) ||
          ...
          pkmn.calc_stats
          pkmn.name           = _INTL("Egg")
          pkmn.steps_to_hatch = pkmn.species_data.hatch_cycles * Settings::CYCLE_STEPS
          pkmn.hatched_map    = 0
