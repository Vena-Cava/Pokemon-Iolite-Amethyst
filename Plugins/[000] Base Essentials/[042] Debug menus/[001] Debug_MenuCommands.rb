...
MenuHandlers.add(:debug_menu, :fill_boxes_abilities, {
  "name"        => _INTL("Fill storage with Abilities"),
  "parent"      => :pokemon_menu,
  "description" => _INTL("Puts one Pokémon with each Ability in storage."),
  "effect"      => proc {
    added = 0
    box_qty = $PokemonStorage.maxPokemon(0)
    completed = true
    ...
    base_species = GameData::Species.keys.first if !GameData::Species.exists?(base_species)
    ...
    GameData::Ability.each do |ability_data|
      ...
      pkmn = Pokemon.new(base_species, 50)
      pkmn.ability = ability_data.id
      pkmn.name = ability_data.real_name if pkmn.respond_to?(:name=)

      $PokemonStorage[(added / box_qty), (added % box_qty)] = pkmn
      added += 1
    end

    pbMessage(_INTL("Storage boxes were filled with Pokémon with one of each Ability."))

    if !completed
      pbMessage(_INTL("Note: There was not enough storage space for every Ability."))
    end
  }
})
