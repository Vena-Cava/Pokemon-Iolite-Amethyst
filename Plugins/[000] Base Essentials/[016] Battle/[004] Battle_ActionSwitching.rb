class Battle
  ...
  def pbEntryHazards(battler)
	...
	# Scattered Toys
	if battler_side.effects[PBEffects::ScatteredToys] && battler.takesIndirectDamage? &&
	   GameData::Type.exists?(:FAIRY) && !battler.airborne? && !battler.hasActiveItem?(:HEAVYDUTYBOOTS)

	  bTypes = battler.pbTypes(true)
	  eff = Effectiveness.calculate(:FAIRY, *bTypes)

	  if !Effectiveness.ineffective?(eff)
		battler.pbReduceHP(battler.totalhp * eff / 8, false)
		pbDisplay(_INTL("Plastic toys dug into {1}!", battler.pbThis))
		battler.pbItemHPHealCheck
	  end
	end
    # Spikes
