...
class PokemonEncounters
  ...
  def encounter_triggered?(enc_type, repel_active = false, triggered_by_step = true)
    ...
    if first_pkmn
      ...
      when :PUREINCENSE
        ...
        when :STENCH, :WHITESMOKE, :QUICKFEET, :ZEROTOHERO
          encounter_chance /= 2
          min_steps_needed *= 2
        when :INFILTRATOR
        ...
        when :SNOWCLOAK
          if GameData::Weather.get($game_screen.weather_type).category == :Snow
            encounter_chance /= 2
            min_steps_needed *= 2
          end
        when :SANDVEIL
        ...
        when :SWARM
          encounter_chance *= 1.5
          min_steps_needed /= 2
        when :ILLUMINATE, :ARENATRAP, :NOGUARD, :LINGERINGAROMA
          encounter_chance *= 2
          min_steps_needed /= 2
        when :BIOLUMINESCENCE
          if has_cave_encounters? || !PBDayNight.isDay?(pbGetTimeNow)
            encounter_chance *= 2
            min_steps_needed /= 2
          end
        end
  ...
  def have_double_wild_battle?
    ...
    if first_pkmn
      case first_pkmn.ability_id
      when :COSTAR, :COMMANDER
        return true if rand(100) < 30   # 30% chance
      end
    end
