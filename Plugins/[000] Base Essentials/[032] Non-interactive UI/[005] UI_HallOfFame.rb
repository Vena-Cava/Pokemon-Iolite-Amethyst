...
class HallOfFame_Scene
  ...
  def pbPCSelection
    loop do
      ...
      pbUpdate
      continueScene = true
      break if Keybinds.trigger(:back)   # Exits
      if Keybinds.trigger(:use)   # Moves the selection one entry backward
        @battlerIndex += 10
        continueScene = pbUpdatePC
      end
      if Keybinds.trigger(:left)   # Moves the selection one pokémon forward
        @battlerIndex -= 1
        continueScene = pbUpdatePC
      end
      if Keybinds.trigger(:right)   # Moves the selection one pokémon backward
        @battlerIndex += 1
        continueScene = pbUpdatePC
      end
