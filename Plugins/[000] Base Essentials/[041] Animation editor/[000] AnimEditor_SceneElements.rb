module BattleAnimationEditor
  ...
  def pbTrackPopupMenu(commands)
    ...
    loop do
      ...
      if Keybinds.trigger(:back)   # Escape
        break
      end
    end
    menuwindow.dispose
    ...
    def updateInput
      ...
      if cel
        ...
        if !self.locked?(@currentcel)
          ...
          if Keybinds.trigger(:down) || Keybinds.repeat?(:down)
            increment = (Input.press?(Input::ALT)) ? 1 : 8
            cel[AnimFrame::Y] += increment
            @dirty[@currentcel] = true
          end
          if Keybinds.trigger(:left) || Keybinds.repeat?(:left)
            increment = (Input.press?(Input::ALT)) ? 1 : 8
            cel[AnimFrame::X] -= increment
            @dirty[@currentcel] = true
          end
          if Keybinds.trigger(:right) || Keybinds.repeat?(:right)
            increment = (Input.press?(Input::ALT)) ? 1 : 8
            cel[AnimFrame::X] += increment
            @dirty[@currentcel] = true
          end
