...
class PurifyChamberSetView < Sprite
  ...
  def moveCursor(button)
    ...
    if @cursor == 0 && points > 0
      @cursor = 1 if button == :up
      @cursor = (points * 1 / 4) + 1 if button == :right
      @cursor = (points * 2 / 4) + 1 if button == :down
      @cursor = (points * 3 / 4) + 1 if button == :left
    elsif @cursor > 0
      pos = @cursor - 1
      ...
      case seg
      when 7, 0
        pos -= 1 if button == :left
        pos += 1 if button == :right
        pos = nil if button == :down
      when 1, 2
        pos -= 1 if button == :up
        pos += 1 if button == :down
        pos = nil if button == :left
      when 3, 4
        pos -= 1 if button == :right
        pos += 1 if button == :left
        pos = nil if button == :up
      when 5, 6
        pos -= 1 if button == :down
        pos += 1 if button == :up
        pos = nil if button == :right
      end
      if pos.nil?
        @cursor = 0
      else
  ...
  def pbSetScreen
    pbDeactivateWindows(@sprites) do
      loop do
        ...
        pbUpdate
        btn = 0
        btn = :down if Keybinds.repeat?(:down)
        btn = :up if Keybinds.repeat?(:up)
        btn = :right if Keybinds.repeat?(:right)
        btn = :left if Keybinds.repeat?(:left)
        if btn != 0
          pbPlayCursorSE
