...
class PokemonRegionMap_Scene
  ...
  def pbMapScene
    ...
    loop do
      ...
      elsif Keybinds.press?(:use) && @mode == 1   # Choosing an area to fly to
      ...
      elsif Keybinds.press?(:use) && @editor   # Intentionally after other USE input check
        pbChangeMapLocation(@map_x, @map_y)
      elsif Keybinds.press?(:action) && !@wallmap && !@fly_map && pbCanFly?
        pbPlayDecisionSE
        @mode = (@mode == 1) ? 0 : 1
        refresh_fly_screen
      end
