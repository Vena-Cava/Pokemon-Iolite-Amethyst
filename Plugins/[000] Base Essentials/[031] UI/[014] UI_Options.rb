...
class PokemonSystem
  ...
  def initialize
	...
	@controller_layout = 0 # 0=Xbox, 1=PlayStation, 2=Nintendo
  end
end
...
class ButtonOption
  ...
  def initialize(name, description, proc)
    @name        = name
...
class Window_PokemonOption < Window_DrawableCommand
  ...
  def drawItem(index, _count, rect)
    ...
    when SliderOption
      ...
      area_width = rect.width - optionwidth

      xpos = optionwidth + rect.x + ((area_width - value_width) / 2)

      pbDrawShadowText(self.contents, xpos, rect.y, value_width, rect.height, value,
                       SEL_VALUE_BASE_COLOR, SEL_VALUE_SHADOW_COLOR)
    end
  end
  ...
  def update
    ...
    dorefresh = true if self.index != oldindex
...
class PokemonOption_Scene
  ...
  def pbStartScene(in_load_screen = false)
    ...
    MenuHandlers.each_available(:options_menu) do |option, hash, name|
	  if hash["type"] == ButtonOption
	    @options.push(
		  ButtonOption.new(name, hash["description"], hash["button_proc"])
	    )
	  else
	    @options.push(
    ...
    pbActivateWindow(@sprites, "option") do
      ...
      loop do
        ...
        if Keybinds.press?(:back)
		  ...
		  current = @options[@sprites["option"].index]

		  if current.is_a?(ButtonOption)
			current.activate
			@sprites["option"].refresh
			pbChangeSelection
		  end
...
MenuHandlers.add(:options_menu, :keyboard_bindings, {
  "name"        => _INTL("Keyboard Bindings"),
  "order"       => 25,
  "type"        => ButtonOption,
  "parameters"  => nil,
  "description" => _INTL("Change keyboard controls."),
  "get_proc"    => proc { next 0 },
  "set_proc"    => proc { |_value, _scene| },
  "on_select"   => proc { |_scene| },
  "button_proc" => proc {
    pbFadeOutIn {
      scene = KeybindListScene.new(:keyboard)
      screen = KeybindListScreen.new(scene)
      screen.pbStartScreen
    }
  }
})

MenuHandlers.add(:options_menu, :controller_bindings, {
  "name"        => _INTL("Controller Bindings"),
  "order"       => 26,
  "type"        => ButtonOption,
  "parameters"  => nil,
  "description" => _INTL("Change controller controls."),
  "get_proc"    => proc { next 0 },
  "set_proc"    => proc { |_value, _scene| },
  "on_select"   => proc { |_scene| },
  "button_proc" => proc {
    pbFadeOutIn {
      scene = KeybindListScene.new(:controller)
      screen = KeybindListScreen.new(scene)
      screen.pbStartScreen
    }
  }
...
MenuHandlers.add(:options_menu, :controller_layout, {
  "name"        => _INTL("Controller Layout"),
  "order"       => 27,
  "type"        => EnumOption,
  "parameters"  => [_INTL("Xbox"), _INTL("PlayStation"), _INTL("Nintendo")],
  "description" => _INTL("Choose which controller button names are shown."),
  "get_proc"    => proc { next $PokemonSystem.controller_layout },
  "set_proc"    => proc { |value, _scene| $PokemonSystem.controller_layout = value }
})
