...
class PokemonStorageScene
  ...
  def pbSelectBoxInternal(_party)
    ...
    loop do
      ...
      if Keybinds.press?(:jumpup)
        ...
        pbUpdateOverlay(selection)
        pbSetMosaic(selection)
      elsif Keybinds.press?(:special)   # Jump to box name
        if selection != -1
          pbPlayCursorSE
          selection = -1
          pbSetArrow(@sprites["arrow"], selection)
      ...
      elsif Keybinds.press?(:action) && @command == 0   # Organize only
        pbPlayDecisionSE
  ...
  def pbSelectPartyInternal(party, depositing)
    ...
    loop do
      ...
      self.update
      if Keybinds.press?(:action) && @command == 0   # Organize only
        pbPlayDecisionSE
        pbSetQuickSwap(!@quickswap)
      elsif Keybinds.press?(:back)
        @selection = selection
