...
class Battle::Scene::PokemonDataBox < Sprite
  ...
  MALE_BASE_COLOR         = Color.new(103, 159, 224)
  MALE_SHADOW_COLOR       = Color.new(16, 79, 150)
  FEMALE_BASE_COLOR       = Color.new(255, 124, 109)
  FEMALE_SHADOW_COLOR     = Color.new(168, 53, 40)
  ...
  def initializeDataBoxGraphic(sideSize)
	...
	if IASummary::IAVERSION == 1 # Is Pokémon Amethyst
      bgFilename = [_INTL("Graphics/UI/Battle/databox_normal_am"),
                    _INTL("Graphics/UI/Battle/databox_normal_foe_am")][@battler.index % 2]
      if onPlayerSide
        @show_hp_numbers = true
        @show_exp_bar    = true
      end
	elsif IASummary::IAVERSION == 2 # Is Pokémon Iolite
	bgFilename = [_INTL("Graphics/UI/Battle/databox_normal_io"),
                    _INTL("Graphics/UI/Battle/databox_normal_foe_io")][@battler.index % 2]
      if onPlayerSide
        @show_hp_numbers = true
        @show_exp_bar    = true
      end
	  ...
	  if IASummary::IAVERSION == 1 # Is Pokémon Amethyst
        bgFilename = [_INTL("Graphics/UI/Battle/databox_thin_am"),
                      _INTL("Graphics/UI/Battle/databox_normal_foe_am")][@battler.index % 2]
	  elsif IASummary::IAVERSION == 2 # Is Pokémon Iolite
	    bgFilename = [_INTL("Graphics/UI/Battle/databox_thin_io"),
                      _INTL("Graphics/UI/Battle/databox_normal_foe_io")][@battler.index % 2]
	  end
    end
    ...
    when 2
      ...
      @spriteY += [-52, -22, 18, 50][@battler.index]
    when 3
      @spriteX += [-12,  12, -6,  6,  0,  0][@battler.index]
      @spriteY += [-122, # Player Top
             	   -26, # Opp Top
				   -52, # Player Mid
				   44, # Opp Mid
				   18, # Player Bum
  ...
  def initializeOtherGraphics(viewport)
    ...
    hp_bar_file = onPlayerSide ? "overlay_hp" : "overlay_hp_foe"
    @hpBarBitmap = AnimatedBitmap.new("Graphics/UI/Battle/#{hp_bar_file}")
    @expBarBitmap  = AnimatedBitmap.new("Graphics/UI/Battle/overlay_exp")
    # Create sprite to draw HP numbers on
    @hpNumbers = BitmapSprite.new(144, 16, viewport)
#    pbSetSmallFont(@hpNumbers.bitmap)
  ...
  onPlayerSide = @battler.index.even?
    super
    @hpBar.x     = value + @spriteBaseX + 4
	if !onPlayerSide
	  @hpBar.x     = value + @spriteBaseX + 12
	end
    @expBar.x    = value + @spriteBaseX + 10
  ...
  def draw_name
    ...
    pbDrawTextPositions(self.bitmap, [[@battler.name, @spriteBaseX + 8 - nameOffset, 16, :left,
                                       NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
    )
	else
	pbDrawTextPositions(self.bitmap, [[@battler.name, @spriteBaseX + 16 - nameOffset, 16, :left,
                                       NAME_BASE_COLOR, NAME_SHADOW_COLOR]]
    )
	end
  end
  ...
  def draw_level
    if @battler.index.even?
      # "Lv" graphic
      pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/overlay_lv"), @spriteBaseX + 140, 24]])
      # Level number
      pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 162, 24)
    else
      # "Lv" graphic
      pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/overlay_lv"), @spriteBaseX + 148, 22]])
      # Level number
      pbDrawNumber(@battler.level, self.bitmap, @spriteBaseX + 170, 22)
    end
  end
  ...
  def draw_gender
	...
	if @battler.index.even?
      pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 126, 16, :left, base_color, shadow_color]])
	else
	  pbDrawTextPositions(self.bitmap, [[gender_text, @spriteBaseX + 134, 16, :left, base_color, shadow_color]])
	end
  end
  ...
  def draw_status
    ...
    return if s < 0
	if @battler.index.even?
      pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/icon_statuses"), @spriteBaseX + 26, 58,
                                        0, s * STATUS_ICON_HEIGHT, -1, STATUS_ICON_HEIGHT]])
	else
	  pbDrawImagePositions(self.bitmap, [[_INTL("Graphics/UI/Battle/icon_statuses"), @spriteBaseX + 64, 52,
                                        0, s * STATUS_ICON_HEIGHT, -1, STATUS_ICON_HEIGHT]])
	end
  end
  ...
  def draw_shiny_icon
    ...
    shiny_x = (@battler.opposes?(0)) ? 206 : -12   # Foe's/player's
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/shiny", @spriteBaseX + shiny_x, 58]])
  end
  ...
  def draw_special_form_icon
    ...
    if @battler.mega?
	  if @battler.index.even?
        pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_mega", @spriteBaseX - 22, 12]])
	  else
	    pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_mega", @spriteBaseX - 256, 12]])
	  end
      ...
      primalX = (@battler.opposes?) ? 210 : -42   # Foe's/player's
      pbDrawImagePositions(self.bitmap, [[filename, @spriteBaseX + primalX, 8]]) if filename
    end
  end
  ...
  def draw_special_form_icon
	if @battler.tera?
      specialX = (@battler.opposes?(0)) ? 209 : -39   # Foe's/player's
      path = Settings::TERASTAL_GRAPHICS_PATH + "tera_types"
      type_number = GameData::Type.get(@battler.tera_type).icon_position
      pbDrawImagePositions(self.bitmap, [[path, @spriteBaseX + specialX, 10, 0, type_number * 32, 32, 32]])
    else
  ...
  def draw_special_form_icon
	if @battler.ultra?
      specialX = (@battler.opposes?(0)) ? 206 : -38   # Foe's/player's
      pbDrawImagePositions(self.bitmap, [[Settings::ZMOVE_GRAPHICS_PATH + "icon_ultra", @spriteBaseX + specialX, 10]])
    else
  ...
  def draw_special_form_icon
    if @battler.dynamax?
      specialX = (@battler.opposes?(0)) ? 212 : -44   # Foe's/player's	
      pbDrawImagePositions(self.bitmap, [[Settings::DYNAMAX_GRAPHICS_PATH + "icon_dynamax", @spriteBaseX + specialX, 8]])
    else
  ...
  def draw_owned_icon
    ...
    pbDrawImagePositions(self.bitmap, [["Graphics/UI/Battle/icon_own", @spriteBaseX + 8, 54]])
  end
...
def refresh_hp
  ...
  # Show HP numbers
  if @show_hp_numbers
    pbDrawNumber(self.hp, @hpNumbers.bitmap, 64, 2, :right)
    pbDrawNumber(-1, @hpNumbers.bitmap, 64, 2)   # / char
    pbDrawNumber(@battler.totalhp, @hpNumbers.bitmap, 80, 2)
  end

  # Calculate HP bar width (crop from the RIGHT, anchored on the LEFT)
  full_w = @hpBarBitmap.width
  w = 0
  if self.hp > 0
    w = full_w.to_f * self.hp / @battler.totalhp
    w = 1 if w < 1
    # Snap to nearest 2 pixels (for doubled-size graphics)
    w = ((w / 2.0).round) * 2
    w = full_w if w > full_w
  end

  # Set bar color row (green/yellow/red)
  hpColor = 0                                      # Green
  hpColor = 1 if self.hp <= @battler.totalhp / 2   # Yellow
  hpColor = 2 if self.hp <= @battler.totalhp / 4   # Red
  bar_h = @hpBar.src_rect.height
  src_y = hpColor * (@hpBarBitmap.height / 3)

  # IMPORTANT: Crop from right by shrinking width only (do NOT shift src_rect.x)
  @hpBar.src_rect.set(0, src_y, w, bar_h)
end
...
class Battle::Scene::AbilitySplashBar < Sprite
  ...
  def initialize(side, viewport = nil)
    ...
    pbSetSystemFont(self.bitmap)
    # Position the bar
    self.x       = (side == 0) ? -Graphics.width / 2 : Graphics.width + 84
