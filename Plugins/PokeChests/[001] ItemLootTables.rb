...
module LootTables
  ...
  FishingOldTable = {
    :common    => [[:PEARL, 1], [:SHOALSHELL, 1], [:SHOALSALT, 1]],
    :uncommon  => [[:STARDUST, 1], [:HEARTSCALE, 1]],
    :rare      => [[:BIGPEARL, 1]],
    :very_rare => [[:DIVEBALL, 1]]
  }

  FishingGoodTable = {
    :common    => [[:PEARL, 1, 2], [:STARDUST, 1], [:HEARTSCALE, 1]],
    :uncommon  => [[:BIGPEARL, 1], [:NETBALL, 1, 2], [:DIVEBALL, 1, 2]],
    :rare      => [[:STARPIECE, 1], [:WATERSTONE, 1]],
    :very_rare => [[:PEARLSTRING, 1], [:MYSTICWATER, 1]]
  }

  FishingSuperTable = {
    :common    => [[:BIGPEARL, 1], [:STARDUST, 1, 2], [:DIVEBALL, 1, 2]],
    :uncommon  => [[:STARPIECE, 1], [:PEARLSTRING, 1], [:DAMPROCK, 1]],
    :rare      => [[:COMETSHARD, 1], [:PRISMSCALE, 1], [:LUREBALL, 1]],
    :very_rare => [[:BOTTLECAP, 1], [:DEEPSEASCALE, 1]]
  }
