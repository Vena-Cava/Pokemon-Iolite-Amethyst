...
class DiegoWTsStarterSelection
  def initialize(pkmn1,pkmn2,pkmn3)
    ...
    @sprites["shadow_1"] = IconSprite.new(130,212,@viewport)
    @sprites["shadow_2"] = IconSprite.new(296,212,@viewport)
    @sprites["shadow_3"] = IconSprite.new(462,212,@viewport)
    for i in 1..3
      @sprites["shadow_#{i}"].setBitmap("Graphics/Pictures/DiegoWT's Starter Selection/shadow")
      @sprites["shadow_#{i}"].opacity=0
    end
    
    @sprites["ball_1"] = IconSprite.new(176,188,@viewport)
    @sprites["ball_2"] = IconSprite.new(342,188,@viewport)
    @sprites["ball_3"] = IconSprite.new(508,188,@viewport)
    for i in 1..3
    ...
    @sprites["select"] = IconSprite.new(176,188,@viewport) # Outline selection
    ...
    @sprites["pkmn_1"].x = StarterSelSettings::STARTER1X + 342
    @sprites["pkmn_1"].y = StarterSelSettings::STARTER1Y + 148
    @sprites["pkmn_2"].x = StarterSelSettings::STARTER2X + 342
    @sprites["pkmn_2"].y = StarterSelSettings::STARTER2Y + 148
    @sprites["pkmn_3"].x = StarterSelSettings::STARTER3X + 342
    ...
    for i in 1..2
      @sprites["choice#{i}"] = IconSprite.new(542,(i-1)*46+174,@viewport)
      if StarterSelSettings::INSTYLE == 2 # Checks for interface style
    ...
    @sprites["choicesel"] = IconSprite.new(542,174,@viewport)
    if StarterSelSettings::INSTYLE == 2 # Checks for interface style
  ...
  def pbStartChoosing
    ...
    while @endscene == 0
      ...
      if Input.trigger?(Input::RIGHT) || Input.trigger?(Input::LEFT) ||
        ...
        @sprites["selection"].y = @y[@select] - 160
        @sprites["select"].angle = 0
        pbChoosingScene
      end
    end
  ...
  def pbChoosingScene
    ...
    while @endscene == 0
      ...
      if Input.trigger?(Input::RIGHT) && @select < 3
        ...
        @frame = 0
        pbPlayCursorSE
        @sprites["ball_1"].x = 176 if @oldsel == 1
        @sprites["ball_2"].x = 342 if @oldsel == 2
        @sprites["ball_3"].x = 508 if @oldsel == 3
        @sprites["ball_#{@oldsel}"].angle = 0
        @sprites["shadow_1"].x = 130 if @oldsel == 1
        @sprites["shadow_2"].x = 296 if @oldsel == 2
        @sprites["shadow_3"].x = 462 if @oldsel == 3
        @sprites["select"].x = @x[@select]
        @sprites["select"].y = @y[@select]
        @selframe = 0
        @sprites["selection"].y = @y[@select] - 160
    ...
    if switch == 0
      ...
      if StarterSelSettings::INSTYLE == 2 # Checks for interface style
        pbDrawTextPositions(@overlay,[
          ["YES",560,190,0,Color.new(255,255,255),Color.new(140,140,140),false,Graphics.width],
          ["NO",560,236,0,Color.new(255,255,255),Color.new(140,140,140),false,Graphics.width]])
      else
        pbDrawTextPositions(@overlay,[
  ...
  def pbChooseBall
    ...
    20.times do
      pbUpdateSpriteHash(@sprites)
      @sprites["ballbase"].x += 160/20 if @select == 1
      @sprites["ballbase"].x -= 160/20 if @select == 3
    ...
    2.times do
      pbUpdateSpriteHash(@sprites)
      @sprites["ballbase"].x += 3 if @select == 1
      @sprites["ballbase"].x -= 3 if @select == 3
      pbWait(0.01)
    end
