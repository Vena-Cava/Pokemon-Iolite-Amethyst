...
Battle::AI::Handlers::AbilityRanking.add(:LIMITBREAK,
  proc { |ability, score, battler, ai|
    low_pp_move = battler.check_for_move do |m|
      next false if !m.respond_to?(:pp)
      next m.pp <= 1
    end

    near_empty_move = battler.check_for_move do |m|
      next false if !m.respond_to?(:pp)
      next m.pp <= 3
    end

    has_damaging_move = battler.check_for_move { |m| m.damagingMove? }
    has_status_move   = battler.check_for_move { |m| m.statusMove? }

    next score + 2 if low_pp_move && has_damaging_move
    next score + 1 if low_pp_move && has_status_move

    next score + 1 if near_empty_move && has_damaging_move
    next score if near_empty_move && has_status_move

    next score if has_damaging_move
    next score - 1 if has_status_move
...
Battle::AI::Handlers::AbilityRanking.add(:CHIPPEDSTONE,
  proc { |ability, score, battler, ai|
    next score if battler.pbOpposingSide.effects[PBEffects::StealthRock] == 0
    next score - 1
  }
)

Battle::AI::Handlers::AbilityRanking.add(:GLASSSPLINTERS,
  proc { |ability, score, battler, ai|
    next score if battler.pbOpposingSide.effects[PBEffects::Spikes] < 3
    next score - 1
  }
)

Battle::AI::Handlers::AbilityRanking.add(:TONOFBRICKS,
  proc { |ability, score, battler, ai|
    next score if battler.pbOpposingSide.effects[PBEffects::ScatteredToys] == 0
    next score - 1
  }
)

Battle::AI::Handlers::AbilityRanking.add(:GROUNDWIRE,
  proc { |ability, score, battler, ai|
    terrain = battler.battler.battle.field.terrain

    # Electric Terrain is active, so the Pokémon is in its preferred condition.
    next score + 1 if terrain == :Electric

    # Terrain is absent, but the ability can still create it.
    next score
  }
)
