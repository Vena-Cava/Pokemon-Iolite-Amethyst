...
class Battle::Battler
  attr_accessor :serrated_edge_bonus_hit
end

module SerratedEdge
  def self.applies?(user, move, targets)
    return false if !user.hasActiveAbility?(:SERRATEDEDGE)
    return false if !move.pbDamagingMove?
    return false if !move.slicingMove?
    return false if move.chargingTurnMove?
    return false if targets.length != 1
    return true
  end

  def self.patch_pbNumHits(klass)
    return if klass.instance_variable_get(:@serrated_edge_patched)
    klass.instance_variable_set(:@serrated_edge_patched, true)

    klass.alias_method(:serrated_edge_old_pbNumHits, :pbNumHits)

    klass.define_method(:pbNumHits) do |user, targets|
      hits = serrated_edge_old_pbNumHits(user, targets)

      @serrated_edge_bonus_hit_num = nil

      if SerratedEdge.applies?(user, self, targets)
        @serrated_edge_bonus_hit_num = hits
        hits += 1
      end
...
SerratedEdge.patch_pbNumHits(Battle::Move)

# Patch Battle::Move and all move subclasses that override pbNumHits.
ObjectSpace.each_object(Class) do |klass|
  next if !(klass <= Battle::Move)
  next if !klass.instance_methods(false).include?(:pbNumHits)
  SerratedEdge.patch_pbNumHits(klass)
end

#-------------------------------------------------------------------------------
# Mark only the final added hit as the Serrated Edge bonus hit.
#-------------------------------------------------------------------------------
class Battle::Battler
  alias serrated_edge_old_pbProcessMoveHit pbProcessMoveHit
  def pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
    user.serrated_edge_bonus_hit = false

    if move.instance_variable_defined?(:@serrated_edge_bonus_hit_num)
      bonus_hit = move.instance_variable_get(:@serrated_edge_bonus_hit_num)
      user.serrated_edge_bonus_hit = (hitNum == bonus_hit)
    end

    ret = serrated_edge_old_pbProcessMoveHit(move, user, targets, hitNum, skipAccuracyCheck)
...
#-------------------------------------------------------------------------------
# 1/4 damage on the added Serrated Edge hit.
# Does NOT use Parental Bond, so the normal move animation still plays.
#-------------------------------------------------------------------------------
class Battle::Move
  alias serrated_edge_old_pbCalcDamageMultipliers pbCalcDamageMultipliers
  def pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)
    serrated_edge_old_pbCalcDamageMultipliers(user, target, numTargets, type, baseDmg, multipliers)

    if user.serrated_edge_bonus_hit
